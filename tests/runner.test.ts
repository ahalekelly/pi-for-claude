...
test("run creates an isolated worktree and sends the composed prompt through the SDK", (t) => {
  ...
  assert.deepEqual(readdirSync(model.agentDir).sort(), ["auth.json", "models-store.json", "models.json", "pi-for-claude-instructions.md", "settings.json"]);
...
test("run in a git project creates no branch or worktree", (t) => {
  ...
  const sessions = join(root, ".agents/sessions");
  const record = JSON.parse(readFileSync(sessionArtifact(sessions, "change"), "utf8"));
  assert.equal(record.kind, "in-place");
  assert.equal(record.worktree, realpathSync(root));
...
test("a linked worktree is its own project: sessions, pi worktrees, and merge stay inside it", (t) => {
  ...
  const record = JSON.parse(readFileSync(sessionArtifact(join(linked, ".agents/sessions"), "fix-auth"), "utf8"));
...
test("sessions are ordered by metadata creation time", () => {
  const root = scratchRepo("pi-for-claude-session-order-");
  const sessions = join(root, ".agents/sessions");
  mkdirSync(sessions, { recursive: true });
  const record = {
    kind: "review",
...
test("watch binds to the active resume turn and result rejects partial state", async (t) => {
  ...
  const model = startModelServer(root, [
    ...
    { kind: "text", text: "Second result.", delayMs: 5000 },
  ]);
  t.after(model.stop);
