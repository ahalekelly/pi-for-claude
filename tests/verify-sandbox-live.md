...
3. `curl --connect-timeout 5 https://example.com` — this must succeed because network access is unrestricted.
4. `cat "$HOME/.agents/secrets.env"` — this must fail because the file is in `denyRead`.
5. `sleep 30` — this gives the operator time to inspect the running process.
...
- The `curl` call succeeds.
- Reading `~/.agents/secrets.env` fails.
